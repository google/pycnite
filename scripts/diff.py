...
def codetree_from_dis(code, filename):
  ...
  return CodeTree(name=name, opcodes=opcodes, children=children)
...
def codetree_from_pycnite(dis_code):
  ...
  children = [
      codetree_from_pycnite(d)
      for d in dis_code.children
  ]
  return CodeTree(name=name, opcodes=opcodes, children=children)
...
def diff_trees(tree1, tree2):
  ...
  if diff:
    print(f"FAILED: {tree1.name}")
    for d in diff:
      print("  ", d)
  else:
    print(f"PASSED: {tree1.name}")
